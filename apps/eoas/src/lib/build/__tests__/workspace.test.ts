...
it('does not clean up a new build when interruption finishes after normal unwinding', async () => {
  ...
  try {
    ...
    await ready.promise;
    const interrupt = process.listeners('SIGINT').find(listener => !listeners.includes(listener))!;
    interrupt('SIGINT');
